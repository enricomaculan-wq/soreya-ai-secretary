import { jsonError, readMicrosoftOAuthConfig, setOAuthState } from "@/lib/server/calendar-api";
import { getAuthenticatedServerContext } from "@/lib/server/supabase";
import { randomUUID } from "node:crypto";
import { NextResponse } from "next/server";

const MICROSOFT_SCOPES = ["offline_access", "User.Read", "Calendars.Read"];
...
export async function GET() {
  try {
    ...
    await setOAuthState("microsoft", state);
