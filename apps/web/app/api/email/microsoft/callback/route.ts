...
const MICROSOFT_MAIL_FALLBACK_SCOPES = ["offline_access", "User.Read", "Mail.Read"];

export async function GET(request: Request) {
  const url = new URL(request.url);

  try {
    await verifyEmailOAuthState("microsoft", url.searchParams.get("state"));
    const code = url.searchParams.get("code");

    if (!code) {
      throw new Error("Missing Microsoft Mail OAuth code.");
    }
    ...
    if (!providerAccountId) {
      throw new Error("Unable to identify Microsoft mail account.");
    }
    ...
    await upsertConnectedEmailAccount(context.supabase, {
      ...
      expiresAt: tokenResponse.expires_in
        ...
        : null,
      scopes: scopesFromTokenResponse(tokenResponse, MICROSOFT_MAIL_FALLBACK_SCOPES),
      status: "active",
      metadata: { oauth_provider: "microsoft_mail" },
    });

    return NextResponse.redirect(new URL("/settings?email=microsoft-connected", request.url));
  } catch (error) {
...
async function exchangeMicrosoftCode(
  code: string,
  config: ReturnType<typeof readMicrosoftMailOAuthConfig>,
  ...
  if (!response.ok) {
    throw new Error(payload.error_description ?? "Microsoft Mail token exchange failed.");
  }
