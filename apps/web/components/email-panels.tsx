...
export function EmailAccountsPanel({ providers, compact = false }: { providers?: EmailProvider[]; compact?: boolean } = {}) {
  ...
  const oauthMessage = useMemo(() => {
    ...
    if (emailResult === "gmail-connected") {
      return t("email.gmailConnected");
    }

    if (emailResult === "gmail-error" && oauthError) {
      return oauthError;
    }
          ...
          return (
            <div key={provider.provider} className="space-y-3">
              <p className="text-sm text-stone-600">
                {connected
                  ? isGoogle
                    ? t("settings.hub.channels.emailGoogle.connected", { email: status?.email ?? "—" })
                    : t("settings.hub.channels.connectedAs", { value: status?.email ?? "—" })
                  : isGoogle
                    ? t("settings.hub.channels.emailGoogle.disconnected")
                    : t("settings.hub.channels.menu.email-microsoftHint")}
              </p>
              <div className="flex flex-wrap gap-2">
                {demoMode ? (
                  ...
                  <a className="soreya-btn-primary px-4 py-2 text-sm" href={provider.connectHref}>
                    {isGoogle ? t("settings.hub.channels.emailGoogle.connect") : t("common.connect")}
                  </a>
                )}
                <button
                  ...
                  type="button"
                >
                  {syncingProvider === provider.provider ? `${t("common.loading")}...` : t("settings.hub.channels.emailGoogle.syncNow")}
                </button>
              </div>
            </div>
          );
